    ...
    <div style={{ backgroundColor: '#fff', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes starBlink { 0%, 100% { opacity: 0.1; transform: scale(0.5); } 50% { opacity: 0.8; transform: scale(1.2); } }
        @keyframes starRise { 0% { transform: translateY(0px); } 100% { transform: translateY(-150px); opacity: 0; } }

        .hero { position: relative; width: 100%; height: 520px; border-bottom: 1px solid #e2e8f0; display: flex; flex-direction: column; align-items: center; justify-content: center; overflow: hidden; background: linear-gradient(-45deg, #eef2ff, #f8fafc, #e0e7ff, #f1f5f9); background-size: 400% 400%; animation: gradientShift 15s ease infinite; }
        ...
        .real-star { position: absolute; z-index: 2; pointer-events: none; filter: drop-shadow(0 0 8px rgba(255, 215, 0, 0.8)); }
        ...
        .search-box { width: 100%; padding: 20px 65px 20px 30px; border-radius: 100px; border: 1px solid rgba(255,255,255,0.6); outline: none; font-size: 16px; box-shadow: 0 20px 40px -10px rgba(0,0,0,0.1); color: #000; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(10px); }
        ...
        .result-bar { background: #3b82f6; color: white; padding: 8px; text-align: center; font-size: 12px; font-weight: bold; text-transform: uppercase; }

        /* ========================================= */
        /* THE GRID - EXACTLY 2 MOBILE, 3 DESKTOP    */
        /* ========================================= */
        .grid-layout { 
          display: grid; 
          grid-template-columns: repeat(2, minmax(0, 1fr)); /* 2 COLUMNS ALWAYS ON MOBILE */
          gap: 15px; 
          padding: 30px 15px; 
          max-width: 1200px; 
          margin: 0 auto; 
        ...
        .card-title { font-size: 14px; font-weight: 800; color: #1e293b; margin-bottom: 8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; width: 100%;}
      ...
      {!activeQuery ? (
        <div className="hero">
          ...
          <div className="real-star s-1">✨</div><div className="real-star s-2">⭐</div><div className="real-star s-3">✨</div>
          <div className="real-star s-4">⭐</div><div className="real-star s-5">✨</div><div className="real-star s-6">⭐</div>
          <div className="floating-icon em-1">🍔</div><div className="floating-icon em-2">🍕</div><div className="floating-icon em-3">🐶</div><div className="floating-icon em-4">✈️</div>

          <nav className="nav-standard">
            <div className="logo" onClick={resetPage}>PNG WORLD</div>
          ...
          <div className="hero-content">
            ...
            <div className="search-container">
              <input className="search-box" placeholder="Search thousands of transparent PNGs..." value={searchInput} onChange={e => setSearchInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} />
        ...
        <>
          <div className="search-mode-header">
            ...
            <div className="search-header-input-container">
                <input className="search-header-input" placeholder="Search png" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} onKeyDown={handleKeyDown}/>
          ...
          <div className="result-bar">Showing results for "{activeQuery}"</div>
        </>
      )}
        ...
        {loading ? (
          ...
          <Link key={img.public_id} href={`/png/${img.public_id}`} target="_blank" className="card">
            <div className="checkerboard">
              <img src={`https://res.cloudinary.com/dic1ahqrn/image/upload/w_500,f_auto/${img.public_id}.png`} alt="png" style={{ maxHeight: '90%', maxWidth: '90%', objectFit: 'contain' }} />
            </div>
