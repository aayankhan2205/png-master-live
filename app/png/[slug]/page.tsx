...
export default function PngPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleResizeDownload = () => {
    if (!img) return;
    // Constructs a Cloudinary URL that resizes and forces a download attachment
    ...
    window.location.href = resizeUrl;
  };
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '40px' }} className="detail-grid">
        ...
        {/* --- LEFT COLUMN: IMAGE & MAIN DOWNLOAD --- */}
        <div>
        ...
        {/* --- RIGHT COLUMN: SIDEBAR (RESIZE TOOL) --- */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '25px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <input 
                type="number" placeholder="Width" value={w} onChange={(e) => setW(e.target.value)}
                style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0', outline: 'none' }} 
              />
              <input 
                type="number" placeholder="Height" value={h} onChange={(e) => setH(e.target.value)}
                style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0', outline: 'none' }} 
              />
            </div>
            <button 
              onClick={handleResizeDownload}
              style={{ width: '100%', marginTop: '15px', padding: '15px', backgroundColor: '#0f172a', color: '#fff', borderRadius: '12px', fontWeight: 'bold', border: 'none', cursor: 'pointer', transition: '0.2s' }}
            >
              Resize & Download
            </button>
          </div>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '25px' }}>
            ...
            <p style={{ fontSize: '12px', color: '#64748b', lineHeight: '1.5' }}>Free for personal and commercial use. No attribution required.</p>
          </div>
